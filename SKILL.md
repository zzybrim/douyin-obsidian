...
description: "抖音内容提取与归档工具（视频 + 图文）。从抖音分享链接解析内容：视频帖子支持无水印下载、语音识别提取口播文案；图文帖子提取正文文字、批量下载图片。结构化归档到 Obsidian vault。触发场景：用户发送 v.douyin.com 链接并要求提取/转录/总结内容；用户说'把这个视频转成文字''扒一下这个帖子''图片里说了什么'；用户要求下载抖音无水印视频或图文内容。当用户需要处理抖音链接、提取文案、或批量整理抖音内容时使用此 skill。"
